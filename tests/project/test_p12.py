...
from GramatykiGrafoweAGH.project.taskC import P12
from GramatykiGrafoweAGH.testing import assert_production_cannot_be_applied
from tests.utils import make_P12_left_side_graph, make_P11_left_side_graph


def make_P12_left_side_graph_not_middle():
...
def test_P12_isomorphic_left_side():
    G = make_P12_left_side_graph()
    ...
    G.is_isomorphic_with(make_P12_right_side_graph())


def test_P12_can_be_applied_only_once():
    G = make_P12_left_side_graph()

    P12(G)

    assert_production_cannot_be_applied(P12, G)


def test_P12_left_side_deleted_node():
    G = make_P12_left_side_graph()
...
def test_P12_left_side_deleted_edge():
    G = make_P12_left_side_graph()
...
def test_P12_left_side_wrong_label():
    G = make_P12_left_side_graph()
...
def test_P12_left_side_subgraph():
    # test application of P12 to the graph with the subgraph isomorphic to left side of P12
    G = make_P12_left_side_graph()
    expected = make_P12_right_side_graph()
...
def test_P12_on_P11_graph():
    G = make_P11_left_side_graph()
    assert_production_cannot_be_applied(P12, G)


def test_P12_left_side_not_middle():
    G = make_P12_left_side_graph_not_middle()
    assert_production_cannot_be_applied(P12, G)
